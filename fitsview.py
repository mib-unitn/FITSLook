...
class FITSViewer(QMainWindow):
    def __init__(self, filepath=None):
        ...
        self.stack.currentChanged.connect(self.on_tab_changed)  # <--- FIX: Monitor tab switches

        if filepath: self.open_file(filepath)

    def on_tab_changed(self, index):
        """Fix layout when switching back to plot view"""
        if index == 0:  # Plot Tab
            # Force Matplotlib to recalculate layout based on visible size
    ...
    def open_file(self, filepath):
        ...
        try:
            ...
            for i, hdu in enumerate(self.hdul):
                # Smart detect 1D spectrum in extension 4 or similar
    ...
    def render_spectrum(self, h):
        ...
        self.ax.margins(x=0)
        self.ax.set_aspect('auto') # Ensure it stretches
    ...
    def render_image(self):
        ...
        self.ax.set_aspect('equal') # Images should be square pixels
        self.fig.tight_layout()
        self.canvas.draw()
